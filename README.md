...
- MCP 工具：通过 FastMCP 暴露仓库树、文件读取、全文搜索、技术栈识别、Git 摘要和报告保存工具。
...
RepoPilot 使用“父目录白名单 + session repo”模型。配置文件中的 `readable_roots` 决定可选择仓库所在的父目录；每次 CLI/WebUI 调用会指定一个具体仓库路径，工具只能读取该仓库内部内容。写权限默认只允许 `outputs/`，并拒绝 `.env`、`.git`、`.venv`、`node_modules`、`__pycache__` 等路径。
