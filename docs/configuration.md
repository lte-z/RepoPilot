...
RepoPilot 的配置分为两类：本地 YAML 配置和环境变量。YAML 负责权限、工具行为和运行限制；环境变量负责 LLM 服务连接。运行时配置默认保存在当前工作目录的 `.repopilot/`，默认文件由 `repopilot setup` 或首次运行 `repopilot` 自动生成。
...
`.repopilot/` 会被 Git 忽略，适合保存个人路径、API Key 和本地报告。

如果需要从其他目录启动 RepoPilot，但仍希望把运行时配置写入某个固定目录，可以设置 `REPOPILOT_PROJECT_ROOT`。移动或复制仓库后应重新创建 `.venv`，不要直接复制虚拟环境。
...
3. 当前工作目录或 `REPOPILOT_PROJECT_ROOT` 下的 `.repopilot/config.yaml`。

相对路径会以配置文件所在目录为基准解析。

`--config` 和 `REPOPILOT_CONFIG` 只影响 YAML 配置。LLM 供应商、Base URL、模型和 API Key 始终从当前运行目录或 `REPOPILOT_PROJECT_ROOT` 下的 `.repopilot/.env` 读取，也可以被同名系统环境变量覆盖。
...
`.repopilot/.env` 保存 OpenAI-compatible 连接项：
