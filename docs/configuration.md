...
`pip uninstall repopilot`、删除 `.venv/` 或删除源码目录都不会自动删除 RepoPilot home。停止使用 RepoPilot 前，可以先查看本机保存的数据：
...
确认后清理配置、API Key、repo profiles 和报告：

```bash
repopilot config clean
```

自动化测试或脚本场景可以使用 `repopilot config clean --yes` 跳过确认。清理逻辑会检查 RepoPilot home marker，并拒绝删除缺少合法 marker 或明显危险的路径。
